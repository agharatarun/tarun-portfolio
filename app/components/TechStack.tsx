  ...
  return (
    <section id="stack" className="py-12">
      <div className="max-w-5xl mx-auto px-6">
        ...
        <p className="reveal text-[var(--text-2)] text-sm mb-8 max-w-[520px] leading-relaxed">
          From cloud-native backend systems to modern reactive UIs — 13 years of deliberate skill-building across the entire stack.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {techCategories.map((cat) => (
            <div key={cat.title} className={`reveal relative overflow-hidden bg-[var(--bg2)] border border-[var(--border)] rounded-2xl p-6 hover:border-[var(--border-hi)] hover:-translate-y-1 hover:shadow-[0_20px_48px_rgba(0,0,0,.35)] transition-all duration-250 ${cat.glowClass}`}>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${cat.iconBg}`}>{iconMap[cat.icon]}</div>
              <h3 className="font-display font-bold text-[var(--text)] text-[0.97rem] mb-1">{cat.title}</h3>
              <p className="font-mono text-[10px] uppercase tracking-widest text-[var(--text-3)] mb-4">{cat.subtitle}</p>
