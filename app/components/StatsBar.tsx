      ...
      ">
        {stats.map((s) => (
          <div
            key={s.label}
            className="
              bg-[var(--bg2)] px-6 py-5 text-center
              hover:bg-[var(--bg3)] transition-colors duration-200
            "
          >
            <div className="font-display font-extrabold text-3xl text-sky-400 leading-none mb-1">
              {s.num}
            </div>
            <div className="font-mono text-[10px] tracking-wider uppercase text-[var(--text-3)]">
              {s.label}
            </div>
          </div>
        ))}
